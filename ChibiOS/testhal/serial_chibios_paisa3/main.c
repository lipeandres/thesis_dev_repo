...
//---USART 2, talks to the ELM327. configured to 38400 Bps 
	...
	while(true)
	{
	//chprintf((BaseSequentialStream *)&SD2,"Reading IMX...\n\r");	
	sdStart(&SD1, NULL);
	imxRead(CommandP);
        sdStop(&SD1);
	clearAnswer(AnswerP);
	//chprintf((BaseSequentialStream *)&SD2,"Reading IMX OK...\n\r");	
        chThdSleepMilliseconds(40);	
	sdStart(&SD2, &config);
        	
	elmWrite(CommandP);	
	//chprintf((BaseSequentialStream *)&SD2,"Writing ELM OK...\n\r");	
	elmRead(AnswerP);
	sdStop(&SD2);
	clearCommand(CommandP);
	//chprintf((BaseSequentialStream *)&SD2,"Reading ELM OK...\n\r");	
	chThdSleepMilliseconds(40);	
	sdStart(&SD1, NULL);	
	imxWrite(AnswerP);
	//chprintf((BaseSequentialStream *)&SD2,"Writing IMX OK...\n\n\r");
}
...
void imxRead	(CommandArray * Command){
	...
	//chprintf((BaseSequentialStream *)&SD2,(char *)Command->text);
	return;
}
...
/*void elmRead (unsigned char t_answer[]){
	int i=0;
	do{		
	chSequentialStreamRead((BaseSequentialStream *)&SD2, 
	ans_i,1);
	t_answer[i]=ans_i[0];
	i++;	
	...
	i=0;
	ans_i[0]=0;
	chprintf((BaseSequentialStream *)&SD2,(char *)t_answer);
...
void elmRead (AnswerArray *Answer){
	...
	i=0;
	ans_i[0]=0;
	//chprintf((BaseSequentialStream *)&SD2,(char *)Answer->answer);
	//chprintf((BaseSequentialStream *)&SD2,"\n\r");
return;		
}

void elmWrite	(CommandArray * Command){		
//chprintf((BaseSequentialStream *)&SD2,Command->command);
chSequentialStreamWrite((BaseSequentialStream *)&SD2,Command->command,5);
//clearArray(Command);
return;
}
